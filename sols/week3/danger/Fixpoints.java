...
public class Fixpoints {
    ...
    public static <T> Set<T> closure(UnaryOperator<Set<T>> f, Set<T> m) {
        return leastFixpoint(x -> union(m, f.apply(x)));
    }

    // Neid kaks funktsiooni saab kombineerida üheks natuke kiiremaks funktsiooniks,
    // kui funktsioon f on distributiivne. Siis ei ole vahet, kas rakendame f tervele hulgale
    // või ainult uuetest elementidest koosnevast hulgast.
    public static <T> Set<T> closureIterate(UnaryOperator<Set<T>> f, Set<T> fringe) {
        Set<T> closure = new HashSet<>();
        while (closure.addAll(fringe)) fringe = f.apply(fringe);
        return closure;
    }
