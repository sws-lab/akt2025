...
public class PointyEvaluatorTest {
   ...
   @Test
    ...
    public void test07_exceptions() {
        checkEvalException(var("a"));
        checkEvalException(addrOf("a"));
        checkEvalException(assign("x", var("a")));
        checkEvalException(assign("x", var("Foo")));
        checkEvalException(assign("x", addrOf("a")));
        checkEvalException(assign("a", var("x")));
    }

    @Test
    public void test08_multiple() {
        checkEval(comma(num(10), var("x"), addrOf("y"), deref(num(3))), Map.of());

        checkEvalException(sub(comma(assign("x", num(5)), num(10)), deref(num(9))));

        checkEval(sub(assign("x", var("y")), assign("y", var("z"))), Map.of("x", 2, "y", 3));
        checkEval(sub(assign("y", var("z")), assign("x", var("y"))), Map.of("x", 3, "y", 3));

        checkEval(comma(assign("x", num(1)), assign("y", deref(num(0)))), Map.of("y", 1));

        // 9. Sama alamavaldist ei väärtustata mitu korda.
        checkEval(deref(assign("x", sub(var("x"), neg(num(1))))), Map.of("x", 2));
        checkEval(comma(assign("x", sub(var("x"), neg(num(1)))), assign("x", sub(var("x"), neg(num(1))))), Map.of("x", 3));
        checkEval(assign("y", assign("x", sub(var("x"), neg(num(1))))), Map.of("x", 2, "y", 2));
    }
